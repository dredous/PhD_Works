...
class MakeCurve():
    ...
    def draw_point(self, ptx, pty, thick = 3, point_color = (0, 0, 0), bold_point = True, fill_point = False):
        if bold_point:
            ...
            if fill_point:
                w = 0
            rect_area = (ptx - self.bold_margin, pty - self.bold_margin, 2 * self.bold_margin, 2 * self.bold_margin)
        ...
        for i in range(ub_loop):
            for t in range(self.ncircles):
                ...
                if update_poly == False:
                    qt_x = self.lagrange_polynomial(vec_x, t=loop_t, vec_t=vec_t)
                    qt_y = self.lagrange_polynomial(vec_y, t=loop_t, vec_t=vec_t)
                    self.draw_point(int(qt_x), int(qt_y), point_color=line_color, thick=thick, bold_point=bold_point)
                    ...
                    if self.poly_function == "spline":
                        ...
                        vec_xsub = vec_x
                        vec_ysub = vec_y
                    qt_x = self.curve_function(vec_xsub, t = loop_t, vec_t = vec_tx)
                    qt_y = self.curve_function(vec_ysub, t = loop_t, vec_t = vec_ty)
                    self.draw_point(int(qt_x), int(qt_y), point_color=line_color, thick=thick, bold_point=bold_point)
                elif self.poly_function == "hermite":
                    if i > 0:
                        tangent_x = [self.tangent[i][0] for i in range(i - 1, i + 1)]
                        tangent_y = [self.tangent[i][1] for i in range(i - 1, i + 1)]
                        if i < npoints - 1:
                            self.mid = True
                        ...
                        qt_x = self.curve_function(vec_xh, t=loop_t, tangent = tangent_x)
                        qt_y = self.curve_function(vec_yh, t=loop_t, tangent = tangent_y)
                        self.draw_point(int(qt_x), int(qt_y), point_color=line_color, thick=thick, bold_point=bold_point)
    ...
    def add_new_point(self):
        ...
        self.pts_x_margin.extend([self.ptx - i for i in range(self.bold_margin)])
        self.pts_x_margin.extend([self.ptx + i for i in range(self.bold_margin)])
        self.pts_y_margin.extend([self.pty - i for i in range(self.bold_margin)])
        self.pts_y_margin.extend([self.pty + i for i in range(self.bold_margin)])
        self.vec_t.append(self.npoints)
        if self.npoints > 1:
            self.tangent[0] = [0,0]
            self.tangent[self.npoints - 1] = [0,0]
            self.tangent.insert(self.npoints - 1, self.tangent_point(self.pts[self.npoints - 2: self.npoints + 1]))
    ...
    def draw_new_point(self, add_point = True):
        ...
        if self.npoints > 0:
            if self.poly_function != None:
                ...
                self.draw_polynomial(self.pts_x, self.pts_y, self.vec_t, line_color= (0, 0, 100))
            else:
    ...
    def find_point(self):
        ptx_index = [int(i / (self.bold_margin * 2)) for i, x_axis in enumerate(self.pts_x_margin) if x_axis == self.ptx]
        pty_index = [int(i / (self.bold_margin * 2)) for i, y_axis in enumerate(self.pts_y_margin) if y_axis == self.pty]
    ...
    def update_line(self, pts_x, pts_y, move_point):
        ...
        for i in range(npoints):
            ...
            if i > 0:
                self.draw_polynomial(pts_x[i - 1 : i + 1], pts_y[i - 1 : i + 1], vec_t = [0, 1], line_color=(255, 0, 0), thick=1, bold_point=False, update_poly=False)
        if self.poly_function == "lagrange":
            if move_point:
    ...
    def show_button(self, msg, button_pos, msg_pos, button_color = (211, 211, 211), text_color = (0, 0, 0), highlight_button = True):
        ...
        if pt_in_button and highlight_button:
            ...
            button_color = tuple(map(lambda i, j: i - j, button_color, make_darker_color))
    ...
    def change_point(self):
        ...
        self.pts_x_margin[lb : ub] = [self.ptx - i for i in range(self.bold_margin)]+[self.ptx + i for i in range(self.bold_margin)]
        self.pts_y_margin[lb : ub] = [self.pty - i for i in range(self.bold_margin)]+[self.pty + i for i in range(self.bold_margin)]
    ...
    def move_point(self):
        ...
        if self.poly_function == "hermite":
            if 1 < self.pressed_point_index[0] < self.npoints - 2:
                pts = self.pts[self.pressed_point_index[0] - 2:self.pressed_point_index[0] + 3]
                pts[2][0] = self.ptx
                pts[2][1] = self.pty
                self.tangent[self.pressed_point_index[0] - 1] = self.tangent_point(pts[0:3])
                self.tangent[self.pressed_point_index[0]] = self.tangent_point(pts[1:4])
                self.tangent[self.pressed_point_index[0] + 1] = self.tangent_point(pts[2:5])
            elif self.pressed_point_index[0] == 1:
                pts = self.pts[0:self.pressed_point_index[0] + 3]
                pts[1][0] = self.ptx
                pts[1][1] = self.pty
                self.tangent[self.pressed_point_index[0]] = self.tangent_point(pts[0:3])
                self.tangent[self.pressed_point_index[0] + 1] = self.tangent_point(pts[1:4])
                ...
                pts = self.pts[self.pressed_point_index[0] - 2:self.pressed_point_index[0] + 1]
                pts[2][0] = self.ptx
                pts[2][1] = self.pty
                self.tangent[self.pressed_point_index[0] - 1] = self.tangent_point(pts[0:3])
        if self.poly_function != None:
    ...
    def make_scroll_bar(self, npoints):
        self.vec_tx_margin = []
        self.vec_tx_margin.extend([self.min_point[0] - i for i in range(self.bold_margin)])
        self.vec_tx_margin.extend([self.min_point[0] + i for i in range(self.bold_margin)])
        self.vec_ty_margin = [self.min_point[1] - i for i in range(self.bold_margin)] + [self.min_point[1] + i for i in range(self.bold_margin)]
        if npoints > 0:
            self.show_scroll_bar()
            self.add_scroll_bar_point()
        self.vec_tx_margin.extend([self.max_point[0] - i for i in range(self.bold_margin)])
        self.vec_tx_margin.extend([self.max_point[0] + i for i in range(self.bold_margin)])

    def add_scroll_bar_point(self):
        npoints = len(self.vec_t)
        for i in range(1, npoints):
            x_scpt = int(self.max_point[0] - ((self.max_point[0] - self.min_point[0]) * (self.vec_t[npoints - 1] - self.vec_t[i]) / (self.vec_t[npoints - 1] - self.vec_t[0])))
            self.draw_point(x_scpt, self.min_point[1], thick = 3, point_color = (105, 105, 105), bold_point = True, fill_point = False)
            self.vec_tx_margin.extend([x_scpt - i for i in range(self.bold_margin)])
            self.vec_tx_margin.extend([x_scpt + i for i in range(self.bold_margin)])

    def show_scroll_bar(self):
        self.option_button(pos=(30, 570, 740, 20), button_color=(255, 255, 255))
        self.draw_point(self.min_point[0], self.min_point[1], thick=3, point_color=(105, 105, 105), bold_point=True, fill_point=False)
        self.draw_point(self.max_point[0], self.max_point[1], thick=3, point_color=(105, 105, 105), bold_point=True, fill_point=False)
        self.draw_polynomial([self.min_point[0], self.max_point[0]], [self.min_point[1], self.max_point[1]], vec_t=[0, 1], line_color=(105, 105, 105), thick=1, update_poly=False)

    def find_vect_point(self):
        ptx_index = [int(i / (self.bold_margin * 2)) for i, x_axis in enumerate(self.vec_tx_margin) if x_axis == self.ptx]
        pty_index = [int(i / (self.bold_margin * 2)) for i, y_axis in enumerate(self.vec_ty_margin) if y_axis == self.pty]
    ...
    def change_vect(self):
        ...
        self.vec_t[press_id] = self.vec_t[npoints - 1] - ((self.max_point[0] - self.ptx) * (self.vec_t[npoints - 1] - self.vec_t[0]) / (self.max_point[0] - self.min_point[0]))
        lb = press_id * self.bold_margin * 2
        ub = (press_id + 1) * self.bold_margin * 2
        self.vec_tx_margin[lb : ub] = [self.vec_t[press_id] - i for i in range(self.bold_margin)]+[self.ptx + i for i in range(self.bold_margin)]
    ...
    def show_menu_button(self):
        self.show_button(msg="View", button_pos=self.menu_button_pos, msg_pos=(675, 15), button_color=(161, 161, 161), highlight_button=False)
        self.show_button(msg="Reset", button_pos=self.clear_button_pos, msg_pos=(695, 43))
        self.show_button(msg="Clear Line", button_pos=self.clear_line_button_pos, msg_pos=(695, 71))
        self.show_button(msg="Show Line", button_pos=self.show_line_button_pos, msg_pos=(695, 99))
        self.show_button(msg="Clear Curve", button_pos=self.linear_button_pos, msg_pos=(695, 127))

        self.show_button(msg="Curve Options", button_pos=self.poly_button_pos, msg_pos=(675, 190), button_color=(161, 161, 161), highlight_button=False)
        self.show_button(msg="Lagrange", button_pos=self.lagrange_button_pos, msg_pos=(695, 218))
        self.show_button(msg="Bezier", button_pos=self.bezier_button_pos, msg_pos=(695, 246))
        self.show_button(msg="Hermite", button_pos=self.hermite_button_pos, msg_pos=(695, 274))
        self.show_button(msg="Spline", button_pos=self.spline_button_pos, msg_pos=(695, 302))

        live_pos = "Current position: ("+str(self.ptx)+","+str(self.pty)+")"
        self.show_caption(live_pos, (10, 10, 225, 15))
        self.show_caption("Tips: Double click to move point", (10, 30, 225, 15))

    def show_caption(self, msg, pos):
        self.option_button(pos, button_color=(255, 255, 255))
        self.show_button(msg, pos, pos[0:2], button_color=(255, 255, 255), highlight_button=False)
    ...
    def run(self):
        ...
        while not self.done:
            ...
            if self.show_delete_button:
                self.show_button(msg="Delete", button_pos=self.delete_button_pos, msg_pos=(695, 155))
